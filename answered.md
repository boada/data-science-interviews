...
Latent Dirichlet Allocation (LDA) is an NLP technique that is used for topic modeling. LDA assumes that each of the documents in the input corpus are generated by some small number of topics, out of a set of topics with a pre-set size. The goal is to identify which topics these are-- that is, for each document, to identify which topics generated it. LDA does this by treating each document as a bag of words, and then calculating which topics are likeliest to have generated the words that each document contains, in the frequencies in which that document contains them. 
...
**Gibbs Sampling**:
Here is a quick explanation of how that works, inspired by [this](https://www.youtube.com/watch?v=BaM1uiCpj_E) video.
